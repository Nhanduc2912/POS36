...
Hệ thống cung cấp các file BAT trong thư mục `scripts/` để tự động hóa quá trình chạy và cài đặt, giúp cấu trúc dự án gọn gàng hơn:

### Danh Sách Các File Scripts:

1. **`scripts\setup.bat`**: Kiểm tra SDK, cài đặt dependencies, tạo database.
   - **Cách dùng**: Chuột phải → Run as Administrator (chạy lần cài đặt đầu tiên)
   
2. **`scripts\run.bat`**: Khởi động đồng thời Backend API (`:5098`) và Frontend (`:5173`).
   - **Cách dùng**: Double click để chạy hệ thống
   
3. **`scripts\stop.bat`**: Dừng nhanh các tiến trình liên quan (node, dotnet).
   - **Cách dùng**: Double click để tắt hoàn toàn hệ thống
   
4. **`scripts\build.bat`**: Tự động build ra bản production cho cả Frontend & Backend.
   - **Cách dùng**: Double click khi cần build deploy

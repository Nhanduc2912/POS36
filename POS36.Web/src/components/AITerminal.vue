<template>
  ...
  <Teleport to="body" :disabled="layoutMode !== 'window' && layoutMode !== 'fullpage'">
    ...
    >
      <!-- TOOLBAR -->
      <div class="ai-toolbar" @mousedown="layoutMode === 'window' && startDrag($event)">
        <!-- Layout mode buttons -->
        <div class="tb-group">
          <button v-for="m in MODES" :key="m.key" class="tb-icon" :class="{active: layoutMode===m.key}"
            :title="m.label" @click="setMode(m.key)">
        ...
        <!-- Mode toggle -->
        <button class="tb-mode-btn" :class="mode" @click="mode = mode==='agent'?'chat':'agent'" :title="'Chế độ: ' + mode">
        ...
        <div class="tb-group">
          <button v-for="(t, key) in THEMES" :key="key"
            class="theme-dot" :class="{active: theme===key}"
            :style="{background: t.accent}" :title="key"
            @click="theme=key" />
        </div>

        <!-- Clear / Close -->
        <div class="tb-group ms-auto">
          <span v-if="isConnected" class="status-dot online" title="Đã kết nối">●</span>
          <span v-else class="status-dot offline" title="Mất kết nối">○</span>
          <button class="tb-icon" @click="clearMessages" title="Xóa lịch sử"><i class="bi bi-trash3"></i></button>
          <button class="tb-icon" @click="$emit('close')" title="Đóng"><i class="bi bi-x-lg"></i></button>
        </div>
      </div>
      ...
      <div class="ai-messages" ref="msgEl">
        ...
        <div v-if="messages.length === 0" class="ai-welcome">
          ...
          <small>Model: {{ selectedModel }} · Chế độ: {{ mode === 'agent' ? 'Agent (Tools)' : 'Chat thuần' }}</small>
        </div>
        ...
        <div v-for="(msg, i) in messages" :key="i" class="msg-row" :class="msg.role">
          ...
          <template v-else-if="msg.role === 'ai'">
            ...
            <div class="msg-bubble ai">
              <div class="msg-text typing-cursor" :class="{typing: msg.typing}" v-html="renderMd(msg.text)" />
              <!-- Approval Box -->
              <div v-if="msg.approval && !msg.approvalDone" class="approval" :class="'risk-'+msg.approval.riskLevel.toLowerCase()">
                <div class="appr-header">
                  <span class="risk-badge">{{ riskEmoji(msg.approval.riskLevel) }} {{ msg.approval.riskLevel }}</span>
                  <code>{{ msg.approval.functionName }}</code>
                </div>
                <pre class="appr-args">{{ fmtArgs(msg.approval.functionArgs) }}</pre>
                <div class="appr-btns">
                  <button class="btn-y" @click="doConfirm(msg, true)" :disabled="loading">✅ Xác nhận (Y)</button>
                  <button class="btn-n" @click="doConfirm(msg, false)" :disabled="loading">❌ Từ chối (N)</button>
                </div>
              </div>
              <!-- Usage stats -->
          ...
          <!-- System -->
          <template v-else>
            <div class="msg-system" v-html="renderMd(msg.text)" :class="{typing: msg.typing}" />
          </template>
        </div>
        ...
        <div v-if="loading" class="msg-row ai">
          <div class="msg-avatar"><i class="bi bi-robot"></i></div>
          <div class="msg-bubble ai"><span class="dots"><span/><span/><span/></span></div>
        </div>
      </div>
      ...
      <div class="ai-input-area">
        <textarea
          ...
          v-model="inputText"
          class="ai-input"
          :placeholder="loading ? 'Đang xử lý...' : 'Gõ lệnh bằng tiếng Việt... (Enter gửi, Shift+Enter xuống dòng)'"
          :disabled="loading"
          rows="1"
          @keydown.enter.exact.prevent="sendMsg"
          @input="autoResize"
        />
        <button class="ai-send" @click="sendMsg" :disabled="loading || !inputText.trim()">
...
const teleportTarget = computed(() =>
  layoutMode.value === "window" || layoutMode.value === "fullpage" ? "body" : "parent"
);
...
const panelStyle = computed(() => {
  ...
  const base = {
    ...
    "--muted": t.muted, "--accent": t.accent, "--border": t.border, "--user-color": t.user,
  };
  if (layoutMode.value === "window") {
    return { ...base, left: winPos.value.x + "px", top: winPos.value.y + "px",
...
const startDrag = (e) => {
  ...
  dragging = true;
  dragStart = { mx: e.clientX, my: e.clientY, px: winPos.value.x, py: winPos.value.y };
  window.addEventListener("mousemove", onDrag);
  window.addEventListener("mouseup", stopDrag);
};
const onDrag = (e) => {
  if (!dragging) return;
  winPos.value = { x: dragStart.px + e.clientX - dragStart.mx, y: dragStart.py + e.clientY - dragStart.my };
};
const stopDrag = () => { dragging = false; window.removeEventListener("mousemove", onDrag); window.removeEventListener("mouseup", stopDrag); };

// Resize bottom
const startResizeH = (e) => {
  resizing = true; resizeStart = { y: e.clientY, h: bottomH.value };
  document.body.style.cursor = "ns-resize"; document.body.style.userSelect = "none";
  window.addEventListener("mousemove", onResizeH); window.addEventListener("mouseup", stopResize);
};
...
// Send
const sendMsg = async () => {
  const p = inputText.value.trim(); if (!p) return;
  inputText.value = "";
...
const scrollDown = () => { if (msgEl.value) msgEl.value.scrollTop = msgEl.value.scrollHeight; };
...
onMounted(() => {
  ...
  if (saved) { layoutMode.value = saved; emit("modeChange", saved); }
  fetchModels();
...
.ai-panel {
  ...
  --accent:#f59e0b; --border:#30363d; --user-color:#1f6feb;
...
.ai-toolbar {
  ...
  flex-shrink:0; cursor:default; user-select:none; flex-wrap:wrap;
}
.mode-window .ai-toolbar { cursor:move; border-radius:12px 12px 0 0; }
...
.tb-icon:hover,.tb-icon.active { background:rgba(255,255,255,.08); color:var(--accent); }
...
.tb-mode-btn {
  ...
  padding:4px 10px; border-radius:20px; border:1px solid; cursor:pointer; transition:.2s;
}
.tb-mode-btn.agent { color:#3fb950; border-color:#3fb950; background:rgba(63,185,80,.1); }
.tb-mode-btn.chat  { color:#388bfd; border-color:#388bfd; background:rgba(56,139,253,.1); }

.theme-dot {
  width:14px; height:14px; border-radius:50%; border:2px solid transparent;
  cursor:pointer; transition:.15s;
}
.theme-dot.active { border-color:var(--text); transform:scale(1.25); }

.status-dot { font-size:11px; font-weight:700; }
.status-dot.online  { color:#3fb950; }
.status-dot.offline { color:#f85149; }
...
.ai-welcome {
  ...
  opacity:.5; text-align:center; gap:4px;
}
.ai-welcome p { font-size:16px; font-weight:700; margin:0; }
.ai-welcome small { font-size:11px; }
...
.msg-bubble.user .msg-text {
  background:var(--user-color); color:#fff;
  padding:8px 12px; border-radius:16px 4px 16px 16px;
}
.msg-bubble.ai .msg-text {
  background:var(--surface); border:1px solid var(--border);
  padding:8px 12px; border-radius:4px 16px 16px 16px; line-height:1.6;
}
.msg-bubble.ai .msg-text code {
  background:rgba(255,255,255,.07); padding:1px 5px; border-radius:4px; color:#ff7b72; font-size:.9em;
}
...
.typing-cursor::after { content:"▋"; animation:blink 1s infinite; opacity:1; }
.typing-cursor:not(.typing)::after { display:none; }
@keyframes blink { 0%,100%{opacity:1}50%{opacity:0} }

/* Stats */
.msg-stats {
  display:flex; gap:8px; flex-wrap:wrap;
  font-size:10px; color:var(--muted); padding:4px 4px 0;
}
.msg-stats span { display:flex; align-items:center; }

/* Approval */
.approval {
  border:1px solid; border-radius:8px; overflow:hidden; margin-top:6px;
}
.risk-low    { border-color:#3fb950; background:rgba(63,185,80,.05); }
.risk-medium { border-color:#d29922; background:rgba(210,153,34,.07); }
.risk-high   { border-color:#f85149; background:rgba(248,81,73,.07); animation:pulseR 1.5s infinite; }
@keyframes pulseR { 0%,100%{box-shadow:0 0 0 0 rgba(248,81,73,.3)} 50%{box-shadow:0 0 0 5px rgba(248,81,73,0)} }

.appr-header { display:flex; align-items:center; gap:8px; padding:6px 10px; background:rgba(255,255,255,.03); }
.risk-badge  { font-size:10px; font-weight:800; }
.appr-header code { font-size:12px; color:var(--accent); background:rgba(255,255,255,.06); padding:1px 6px; border-radius:4px; }
.appr-args   { font-size:11px; color:var(--muted); padding:6px 10px; margin:0; max-height:70px; overflow-y:auto; }
.appr-btns   { display:flex; gap:6px; padding:6px 10px; }
.btn-y { flex:1; background:rgba(63,185,80,.15); border:1px solid #3fb950; color:#3fb950; padding:5px; border-radius:6px; font-size:11px; font-weight:700; cursor:pointer; }
.btn-n { flex:1; background:rgba(248,81,73,.1);  border:1px solid #f85149; color:#f85149; padding:5px; border-radius:6px; font-size:11px; font-weight:700; cursor:pointer; }
.btn-y:hover { background:rgba(63,185,80,.3); }
.btn-n:hover { background:rgba(248,81,73,.25); }
button:disabled { opacity:.4; cursor:not-allowed; }

/* Loading dots */
.dots { display:inline-flex; gap:4px; padding:4px 0; }
.dots span { width:7px; height:7px; border-radius:50%; background:var(--muted); animation:bounce .9s infinite; }

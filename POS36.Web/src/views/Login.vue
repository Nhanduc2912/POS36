...
const siteConfig = ref({ siteName: "POS36", siteLogo: "", loginBgImage: "", slogan: "" });
...
<template>
  <div class="container-fluid p-0 login-container">
    <div class="row g-0 min-vh-100">
      <div class="col-lg-6 d-none d-lg-flex position-relative left-panel" :style="siteConfig.loginBgImage ? { backgroundImage: `url('${siteConfig.loginBgImage}')` } : {}">
        <div
          class="overlay d-flex flex-column justify-content-between p-5 w-100 h-100 text-white"
        >
          <div class="logo-box">
            <router-link to="/" class="text-decoration-none text-white d-inline-flex align-items-center gap-2">
              <img v-if="siteConfig.siteLogo" :src="siteConfig.siteLogo" alt="Logo" style="max-height: 48px; object-fit: contain;" />
              <h2 v-else class="fw-bold tracking-tight text-white m-0">{{ siteConfig.siteName || "POS36" }}</h2>
            </router-link>
          </div>
          <div class="content-box pe-5 mb-5">
            ...
            <p class="fs-6 opacity-75 pe-5 lh-lg">
              {{ siteConfig.slogan || "Dành riêng cho những chủ nhà hàng khát khao sự hoàn mỹ trong từng quy trình vận hành. Tăng tốc phục vụ, tối ưu doanh thu." }}
            </p>
          </div>
        </div>
      </div>
        ...
        <div class="form-wrapper w-100" style="max-width: 420px; padding: 2rem">
          <div class="d-lg-none mb-5 text-center">
            <router-link to="/" class="text-decoration-none">
              <img v-if="siteConfig.siteLogo" :src="siteConfig.siteLogo" alt="Logo" style="max-height: 50px; object-fit: contain;" />
              <h2 v-else class="fw-bold text-orange m-0">{{ siteConfig.siteName || "POS36" }}</h2>
            </router-link>
          </div>

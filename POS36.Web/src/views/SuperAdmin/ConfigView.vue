  ...
  <div class="cfg-root">
    ...
    <div class="cfg-header">
      <div class="cfg-breadcrumb">
        <span class="cfg-bc-parent">Cấu hình</span>
        <i class="bi bi-chevron-right cfg-bc-sep"></i>
        <span class="cfg-bc-current">{{ currentTabLabel }}</span>
      </div>
      <button class="sa-action-btn" @click="saveAll" :disabled="saving" v-if="activeTab !== 'logs'">
    ...
    <!-- ====== Cấu hình chung ====== -->
    <div v-show="activeTab === 'general'" class="cfg-section">
      <div class="cfg-section-title"><i class="bi bi-sliders me-2"></i>Cấu hình chung</div>
      <div class="dash-card">
        <div class="row g-3">
          <div class="col-lg-6">
            <div class="mb-3"><label class="cfg-label">Tên hệ thống</label><input v-model="cfg.General.SiteName" class="sa-input w-100" placeholder="POS36" /></div>
            <div class="mb-3"><label class="cfg-label">Slogan</label><input v-model="cfg.General.Slogan" class="sa-input w-100" placeholder="Giải pháp quản lý nhà hàng..." /></div>
    ...
    <div v-show="activeTab === 'logs'" class="cfg-section">
      ...
      <div class="sa-table-wrap">
        <table class="sa-table">
          <thead><tr><th>Hành động</th><th>Mô tả</th><th>Người thực hiện</th><th>IP</th><th>Thời gian</th><th></th></tr></thead>
          <tbody>
      ...
      <div class="logs-pagination" v-if="logsTotal > 0">
        <div class="logs-page-info">
          Trang <strong>{{ logPage }}</strong> / <strong>{{ totalPages }}</strong> &nbsp;•&nbsp; {{ logsTotal }} bản ghi
        </div>
        <div class="logs-page-btns">
          <button class="pg-btn" :disabled="logPage <= 1" @click="goToPage(1)"><i class="bi bi-chevron-double-left"></i></button>
          <button class="pg-btn" :disabled="logPage <= 1" @click="goToPage(logPage - 1)"><i class="bi bi-chevron-left"></i></button>
          <button v-for="p in pageNumbers" :key="p" class="pg-btn"
            :class="{ 'pg-btn-active': p === logPage, 'pg-dots': p === '...' }"
            :disabled="p === '...'" @click="p !== '...' && goToPage(p)">{{ p }}</button>
          <button class="pg-btn" :disabled="logPage >= totalPages" @click="goToPage(logPage + 1)"><i class="bi bi-chevron-right"></i></button>
          <button class="pg-btn" :disabled="logPage >= totalPages" @click="goToPage(totalPages)"><i class="bi bi-chevron-double-right"></i></button>
        </div>
    ...
    <!-- ====== Thanh toán & Webhook ====== -->
    <div v-show="activeTab === 'payment'" class="cfg-section">
      <div class="cfg-section-title"><i class="bi bi-credit-card me-2"></i>Thanh toán & Webhook</div>
      <div class="row g-3">
        <div class="col-lg-6">
          <div class="dash-card">
            <h6 class="dash-card-title"><i class="bi bi-bank me-2"></i>Thông tin ngân hàng (VietQR)</h6>
            <div class="mb-3"><label class="cfg-label">Mã ngân hàng</label><input v-model="cfg.Payment.BankCode" class="sa-input w-100" placeholder="MBBank" /></div>
            <div class="mb-3"><label class="cfg-label">Số tài khoản</label><input v-model="cfg.Payment.BankAccountNo" class="sa-input w-100" placeholder="0123456789" /></div>
            <div class="mb-3"><label class="cfg-label">Tên chủ tài khoản</label><input v-model="cfg.Payment.BankAccountName" class="sa-input w-100" placeholder="NGUYEN VAN A" /></div>
          </div>
        </div>
        <div class="col-lg-6">
          <div class="dash-card">
            ...
            <div class="mb-3">
              <label class="cfg-label">Webhook Secret Key</label>
    ...
    <!-- ====== Email & SMTP ====== -->
    <div v-show="activeTab === 'email'" class="cfg-section">
      <div class="cfg-section-title"><i class="bi bi-envelope me-2"></i>Email & SMTP</div>
      <div class="row g-3">
        <div class="col-lg-6">
          <div class="dash-card">
            <h6 class="dash-card-title"><i class="bi bi-send me-2"></i>EmailJS (Form liên hệ)</h6>
            <div class="mb-3"><label class="cfg-label">Service ID</label><input v-model="cfg.Email.EmailJsServiceId" class="sa-input w-100" placeholder="service_xxx" /></div>
            <div class="mb-3"><label class="cfg-label">Template ID</label><input v-model="cfg.Email.EmailJsTemplateId" class="sa-input w-100" placeholder="template_xxx" /></div>
            <div class="mb-3">
              <label class="cfg-label">Public Key</label>
              <div class="d-flex gap-2">
        ...
        <div class="col-lg-6">
          <div class="dash-card">
            <h6 class="dash-card-title"><i class="bi bi-server me-2"></i>SMTP Server</h6>
            <div class="row g-2 mb-3">
              <div class="col-8"><label class="cfg-label">SMTP Host</label><input v-model="cfg.Email.SmtpHost" class="sa-input w-100" placeholder="smtp.gmail.com" /></div>
              <div class="col-4"><label class="cfg-label">Port</label><input v-model="cfg.Email.SmtpPort" class="sa-input w-100" placeholder="587" /></div>
            </div>
            <div class="mb-3"><label class="cfg-label">Username</label><input v-model="cfg.Email.SmtpUser" class="sa-input w-100" placeholder="yourmail@gmail.com" /></div>
            <div class="mb-3">
            ...
            <div class="mb-3"><label class="cfg-label">Tên hiển thị khi gửi</label><input v-model="cfg.Email.SmtpFromName" class="sa-input w-100" placeholder="POS36 System" /></div>
          </div>
        </div>
      </div>
    </div>
...
// Sync whenever the route query changes (sidebar dropdown navigation)
watch(() => route.query.tab, (tab) => {
  activeTab.value = tab || "general";
  // Auto-load logs when switching to logs tab
  if ((tab || "general") === "logs" && !logs.value.length) loadLogs();
}, { immediate: false });
...
const currentTabLabel = computed(() => NAV_LABELS[activeTab.value] || "Cấu hình chung");
...
const loadLogs = async () => {
  ...
  try {
    ...
    logsTotal.value = r.data.total || 0;
...
const viewLogDetail = async (id) => {
  try { const r = await axios.get(`/api/CauHinh/nhat-ky/${id}`); logDetail.value = r.data; }
  catch (e) { console.error(e); }
};
...
.cfg-breadcrumb { display: flex; align-items: center; gap: 8px; }
.cfg-bc-parent { color: var(--sa-text-faint); font-size: .82rem; font-weight: 600; }
.cfg-bc-sep { color: var(--sa-text-faint); font-size: .7rem; }
.cfg-bc-current { color: var(--sa-accent); font-size: .9rem; font-weight: 800; }
...
/* ===== SECTION ===== */
.cfg-section { flex: 1; min-height: 0; overflow-y: auto; }
.cfg-section-title {
  font-size: .75rem; font-weight: 800;
  color: var(--sa-text-faint); text-transform: uppercase; letter-spacing: .5px;
  margin-bottom: 16px; display: flex; align-items: center;
}
.cfg-label {
  font-size: .75rem; color: var(--sa-text-faint); font-weight: 700;
...
.logs-pagination {
  ...
  padding: 12px 16px; background: var(--sa-surface);
  border: 1px solid var(--sa-border); border-radius: 10px;
}
...
.pg-btn {
  ...
  color: var(--sa-text-muted); border-radius: 8px; cursor: pointer;
  display: flex; align-items: center; justify-content: center;
  font-size: .82rem; font-weight: 600; transition: .15s; padding: 0 8px;
}
.pg-btn:hover:not(:disabled) { background: var(--sa-nav-hover-bg); color: var(--sa-accent); border-color: rgba(245,158,11,.3); }
.pg-btn-active { background: var(--sa-nav-active-bg) !important; color: var(--sa-accent) !important; border-color: rgba(245,158,11,.4) !important; font-weight: 800 !important; }
...
.log-json {
  ...
  color: var(--sa-text-muted); max-height: 200px; overflow: auto;
}
